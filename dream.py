...
def submit(predictions: np.ndarray, sample_path="sample_submission.json", output_path="pred.json") -> None:
...
def train_and_predict(train_path, test_path, sample_path, output_path, size=4, limit=None) -> None:
    """Train the baseline and write test predictions."""
    train_vectors, labels, _ = tokenize(size, train_path, limit=limit)
    model = RandomForestRegressor(n_estimators=100, bootstrap=False, random_state=0, n_jobs=-1)
...
def main() -> None:
    ...
    train_and_predict("data/train_sequences.txt", "data/test_sequences.txt", "sample_submission.json", "pred.json")
